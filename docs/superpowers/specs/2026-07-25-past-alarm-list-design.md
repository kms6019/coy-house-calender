...
이미 울린 알람이 맞기 때문이다. 반복 이벤트에서도 이게 성립하려면 전개 범위 끝을 `now`가
아니라 `now + (최대 alarmMinutesBefore)`로 잡아야 한다. 그러지 않으면 시작이 미래인 회차가
아예 생성되지 않아 이미 울린 알람이 누락된다.

목록은 화면을 열 때의 시각으로 계산한다. 화면을 열어둔 채 시간이 흘러도 이벤트가 바뀌기
전까지는 갱신되지 않는다. 되짚어보기용 화면이라 그대로 둔다.
